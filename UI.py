...
class WhichStatScreen(Screen):
    def __init__(self, **kwargs):
        ...
        for stat in stats:
            btn = Button(text=stat, size_hint_y=None, height=60)
            btn.bind(on_press=self.load_stat)
            grid.add_widget(btn)
    ...
    def load_stat(self, stat):
        stat_screen = self.manager.get_screen('player_compare')
        self.stat = stat

        # compare_screen = self.manager.get_screen("player_compare")
        # compare_screen.set_main_team(self.team_name)
        # compare_screen.year = self.year
...
class MyApp(App):
    def build(self):
        ...
        sm.add_widget(PlayerCompareScreen(name='player_compare'))
        return sm
